...
for i in 1:length(ms)[1]
    ...
    StoreResult(string(subfolder,"/SublatticeSpin_interacting_eq",HMC_par.equation,"_m_",floor(Integer,par.mass*100),"_runlabel_",runlabel), res_Δn)
    CreateFigure(res_Δn, subfolder,string("SublatticeSpin_interacting_eq",HMC_par.equation,"_m_",floor(Integer,par.mass*100),"_runlabel_",runlabel) ,x_label="sample", y_label=L"$\Delta n$", fmt="-", figure_title=string(L"$\Delta n$ for m = ",par.mass, L" and $\alpha_{eff}$ = ", (300/137)/par.ϵ))
...
savefig(abspath(@__DIR__,string("../results/",extrapolate_folder,"/SublatticeSpin_interacting_eq",HMC_par.equation,"_mass_extrapolation_runlabel_",runlabel,".png")))
